...
struct worker_thread_params {
    ...
    :n(_n), w(_w), port(_p), host(_h), request_buffer(buff), worker_mutex(mut), request_threads(p){
        workerChannel = new NetworkRequestChannel(host, port);
    }
};
...
int main(int argc, char * argv[]) {
    ...
    for (int i = 0; i < w; ++i){
        worker_threads.push_back( worker_thread_params(n, w, PORT, HOST, &request_buffer, &client_mutex, &request_threads));
    }
